...
function renderTeacherEditQuiz() {
  ...
  pageWrap(`${header({back:true, titleBackPage:'teacherQuiz', coins:false})}<h1 class="page-title">${isEdit ? 'Edit Parameter Kuis' : 'Buat Kuis Baru'}</h1><p class="page-subtitle">Formulir pengaturan butir soal instan diagnostic 4-Tier.</p><div class="card"><div class="field"><label>Judul / Topik Kuis</label><input id="qType" class="input" value="${escapeHtml(qData.type)}"></div><div class="field"><label>Target Evaluasi</label><select id="qTier" class="select"><option value="1" ${qData.tier===1?'selected':''}>Tier 1 & 2 (Pernyataan Konsep & Keyakinan)</option><option value="3" ${qData.tier===3?'selected':''}>Tier 3 & 4 (Alasan Ilmiah & Keyakinan)</option></select></div><div class="field"><label>Teks Pertanyaan Utama</label><textarea id="qText" class="textarea">${escapeHtml(qData.question)}</textarea></div><div class="form-grid"><div class="field"><label>Pilihan Jawaban A</label><input id="qOpt0" class="input" value="${escapeHtml(qData.optA)}"></div><div class="field"><label>Pilihan Jawaban B</label><input id="qOpt1" class="input" value="${escapeHtml(qData.optB)}"></div><div class="field"><label>Pilihan Jawaban C</label><input id="qOpt2" class="input" value="${escapeHtml(qData.optC)}"></div><div class="field"><label>Pilihan Jawaban D</label><input id="qOpt3" class="input" value="${escapeHtml(qData.optD)}"></div></div><div class="field"><label>Kunci Jawaban</label><select id="qAns" class="select"><option value="0" ${qData.ans===0?'selected':''}>A</option><option value="1" ${qData.ans===1?'selected':''}>B</option><option value="2" ${qData.ans===2?'selected':''}>C</option><option value="3" ${qData.ans===3?'selected':''}>D</option></select></div><button class="btn full" style="margin-top:14px;" onclick="window.saveQuestion()">Simpan Perubahan Kuis</button></div>`);
}
